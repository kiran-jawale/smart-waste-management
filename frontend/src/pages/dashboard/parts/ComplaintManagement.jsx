...
const ComplaintManagement = ({ complaints, theme, onFetchData, userRole }) => {
  ...
  const filteredComplaints = complaints.filter((c) => {
    const statusMatch = complaintStatusFilter === "all" || c.status === complaintStatusFilter;
    const areaMatch = complaintAreaFilter === "" || c.address.toLowerCase().includes(complaintAreaFilter.toLowerCase());
    ...
    <>
      <div className={`p-6 md:p-8 ${theme === "dark" ? "bg-gray-800" : "bg-white"} rounded-2xl shadow-xl`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <select onChange={(e) => setComplaintStatusFilter(e.target.value)} className={inputStyles}>
        ...
        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
          {filteredComplaints.length > 0 ? (
            filteredComplaints.map((c) => (
              ...
              >
                <button onClick={() => setSelectedComplaint(c)} className="text-left w-full sm:w-auto">
                  <span className={`font-semibold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>{c.subject}</span>
                  <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>{c.address}</p>
                </button>
